...
float getposition(bool direccion){
  if(direccion){
    for(byte i=0;i<=5;i++){
      ...
      digitalWrite(2, i & 0x04  );
      digitalWrite(4, i & 0x08 );
      sensorNormalizado[i]=100*(analogRead(39)-sensoresMin[i])/(sensoresMax[i]-sensoresMin[i]); //MAPEO DE SENSORES
    }
  }
  else{
    for(byte i=0;i<=5;i++){
      ...
      sensorNormalizado[i]=100*(analogRead(39)-sensoresMin[i+6])/(sensoresMax[i+6]-sensoresMin[i+6]); //MAPEO DE SENSORES
    }
  }
  for(int i=0; i<=5;i++){
