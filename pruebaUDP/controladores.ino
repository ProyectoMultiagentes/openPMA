...
void set_fuzzy()
{
 ...
 si_curva_suave_AND_error_mediano->joinWithAND(curva_suave, mediano);
  ...
  // construyendo regla 6
  FuzzyRuleAntecedent *si_recta_AND_error_pequenno = new FuzzyRuleAntecedent();
  si_recta_AND_error_pequenno->joinWithAND(recta, pequenno);

  FuzzyRule *fuzzyRule6 = new FuzzyRule(6, si_recta_AND_error_pequenno, entonces_controlador_doble);
  fuzzy->addFuzzyRule(fuzzyRule6);

  // construyendo regla 7
  FuzzyRuleAntecedent *si_recta_AND_error_mediano = new FuzzyRuleAntecedent();
  si_recta_AND_error_mediano->joinWithAND(recta, mediano);

  FuzzyRule *fuzzyRule7 = new FuzzyRule(7, si_recta_AND_error_mediano, entonces_controlador_doble);
  fuzzy->addFuzzyRule(fuzzyRule7);

  // construyendo regla 8
  FuzzyRuleAntecedent *si_recta_AND_error_grande = new FuzzyRuleAntecedent();
  si_recta_AND_error_grande->joinWithAND(recta, grande);

  FuzzyRule *fuzzyRule8 = new FuzzyRule(8, si_recta_AND_error_grande, entonces_controlador_doblePonderado);
  fuzzy->addFuzzyRule(fuzzyRule8);
  
}

double calcular_fuzzy(double error_d,double curvatura){
/*if(error_d>0){
  return 1;
}
else{*/
  fuzzy->setInput(1, abs(error_d));
  fuzzy->setInput(2, curvatura);
