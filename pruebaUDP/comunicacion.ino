...
void lectura_estado(int len) {
  ...
  if (mensaje == "/estado_predecesor") {
    cadena = "V/" + parar + "/" + String(Input_vel) + "/" + String(vel_ref);
  }
  else {
    cadena = "incorrecto";
  }
...
void udp_monitor() {
  ...
  //if(t_actual>=20){
  cadena = String(EtiquetaRobot) +"," + String(t_actual) +"," + String(Input_d) + "," + String(d_ref) + "," + String(vel_ref) + "," + String(Input_vel) + "," + String(Input_theta) + "," + String(Output_d) + "," + String(Output_vel) + "," + String(Output_theta)+ "," + String(curva)+ "," + String(alpha_fuzzy)+ "," + String(radio_curva);
  cadena.toCharArray(msg, cadena.length() + 1);
